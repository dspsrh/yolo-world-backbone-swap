...
class YOLOWorldHead(YOLOv8Head):
    ...
    def loss(self, img_feats: Tuple[Tensor], txt_feats: Tensor,
        ...
        # Fast version.
        # NOTE (yw_src patch): this fork's ``loss_by_feat`` signature is
        #   (cls, bbox, dist, batch_text_masks, batch_gt_instances, img_metas)
        # but the upstream ``loss`` forgot to forward ``txt_masks`` here, which
        # raised "missing 1 required positional argument: 'batch_img_metas'".
        # ``loss_by_feat`` already handles ``txt_masks is None`` (CLIP backbone
        # with add_mask=False), so pass it through as the 4th positional arg.
